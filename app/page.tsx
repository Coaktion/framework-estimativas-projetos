...
const EXTERNAL_LINKS = {
  SCOPE_TEMPLATES: {
    pacote_horas:    "https://drive.google.com/drive/folders/1lOjcJwhnhIWX9BpzNWNAaYKai7cVNyMV",
    time_materials:  "https://drive.google.com/drive/folders/1kpjvc5AVKVYUmuiroqY3zo-8v70PQXU4",
    pacote_horas_ae: "https://drive.google.com/drive/folders/15U7l3i5ODWHWy_x38-o72BOoFs7kEM6K",
    escopo_fechado:  "",
  },
  DEMOS: {
    PT: "https://pdi-aktienowbr.zendesk.com/",
    EN: "https://pdi-aktienowusa.zendesk.com/",
    ES: "",
  },
} as const;
...
export default async function Home() {
  ...
  const topCardsCount =
    (showScopes ? 1 : 0) + (showAE ? 1 : 0) + (isAdmin ? 1 : 0);
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-16">
      <div className="text-center space-y-4">
        <h1 className="text-6xl font-black text-brand-dark tracking-tighter uppercase font-heading leading-tight">
          Aktie Now <span className="text-brand-accent">Tool Center</span>
        </h1>
        <p className="text-slate-400 text-xs font-black uppercase tracking-[0.3em]">{t('home.tagline')}</p>
      </div>

      {/* Linha superior — cards principais da ferramenta */}
      {topCardsCount > 0 && (
        <div className={`grid grid-cols-1 gap-8 w-full max-w-6xl ${topGridCols}`}>
          {showScopes && (
      ...
      {/* Linha inferior — recursos externos (Templates + Demos) */}
      <HomeClient
        templateLinks={EXTERNAL_LINKS.SCOPE_TEMPLATES}
        demoLinks={EXTERNAL_LINKS.DEMOS}
      />
    </div>
  );
}
